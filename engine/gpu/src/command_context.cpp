...
void CommandContext::imageBarrier(const Image& image, ImageState from, ImageState to) {
    imageBarrier(image.handle(), isDepthFormat(image.format()), from, to);
}

void CommandContext::imageBarrier(VkImage image, bool depthAspect, ImageState from,
                                  ImageState to) {
...
void CommandContext::beginRendering(const RenderingDesc& desc) {
    ...
    for (std::size_t i = 0; i < colors.size(); ++i) {
        ...
        colors[i].imageView = target.view;
...
void CommandContext::endRendering() { vkCmdEndRendering(cmd_); }
...
void CommandContext::bindDescriptorSet(const Pipeline& pipeline, VkDescriptorSet set) {
...
void CommandContext::pushConstants(const Pipeline& pipeline, const void* data,
    ...
    vkCmdPushConstants(cmd_, pipeline.layout(), stages, 0, bytes, data);
}

void CommandContext::bindVertexBuffer(VkBuffer buffer, std::uint64_t offset) {
    const VkDeviceSize vkOffset = offset;
    vkCmdBindVertexBuffers(cmd_, 0, 1, &buffer, &vkOffset);
}

void CommandContext::bindIndexBuffer(VkBuffer buffer, std::uint64_t offset) {
    vkCmdBindIndexBuffer(cmd_, buffer, offset, VK_INDEX_TYPE_UINT32);
}
...
void CommandContext::drawIndirect(VkBuffer buffer, std::uint64_t offset, std::uint32_t drawCount,
                                  std::uint32_t stride) {
    vkCmdDrawIndirect(cmd_, buffer, offset, drawCount,
                      stride == 0 ? sizeof(VkDrawIndirectCommand) : stride);
}

void CommandContext::drawIndexedIndirect(VkBuffer buffer, std::uint64_t offset,
                                         std::uint32_t drawCount, std::uint32_t stride) {
    vkCmdDrawIndexedIndirect(cmd_, buffer, offset, drawCount,
