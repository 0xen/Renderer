...
Result<std::unique_ptr<DescriptorTable>> DescriptorTable::create(const Device& device,
    ...
    bindings.push_back({.binding = 26,
                        ...
                        .stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT |
                                      VK_SHADER_STAGE_COMPUTE_BIT});
    bindingFlags.push_back(VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT);
    // Refined OBB table (binding 33): the obb refine dispatch writes rows
    // (compute), the cull dispatch reads them (compute) and the occlusion
    // proxy pass's vertex shader generates oriented boxes from them.
    bindings.push_back({.binding = 33,
                        .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                        .descriptorCount = 1,
                        .stageFlags = VK_SHADER_STAGE_COMPUTE_BIT |
    ...
    // proxy pass's vertex shader resolves each canonical instance row to
    // its entry's local bounds through it (0xffffffff = dead row).
    bindings.push_back({.binding = 34,
                        .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                        .descriptorCount = 1,
                        .stageFlags = VK_SHADER_STAGE_VERTEX_BIT});
    bindingFlags.push_back(VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT);
    ...
    bindings.push_back({.binding = 35,
                        ...
                        .stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT});
    bindingFlags.push_back(VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT);
    // LDR intermediate (binding 36): the post pass's tonemapped output
    // when an AA module is active; the AA pass samples it. Same
    // device-idle rewrite rules as 35.
    bindings.push_back({.binding = 36,
                        .descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
                        .descriptorCount = 1,
                        .stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT});
    bindingFlags.push_back(VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT);
